...
def createParser ():
    ...
    parser.add_argument ('-s', '--nof_stations', nargs='?', type=int, default=100, help='Число базовых станций')
    parser.add_argument ('-c', '--nof_clients', nargs='?', type=int, default=1000000, help='Число клиентов')
    parser.add_argument ('-x', '--max_x', nargs='?', type=int, default=100, help='Максимум по оси X для сетки')
    parser.add_argument ('-y', '--max_y', nargs='?', type=int, default=100, help='Максимум по оси Y для сетки')
    ...
    parser.add_argument ('-b', '--bound', nargs='?', type=int, default=900000, help='Минимальное число обслуживаемых клиентов (Граница для весов)')
...
while counter < nof_tests:
	...
	if nof_clients - nof_zeros < min_weights_sum:
		print('ERROR Sum of weights is too low')
		continue
